...
def _criar_aba(wb, titulo, forn_nome, rede_nome, estados, periodo,
    ...
    # ── Linhas 2+: Informações (A-D) + área de logos (E-G) ───────────────────
    info_rows = [
        ("REDE:", rede_nome),
    ...
    logo_h = n_info * 24  # altura px aproximada (18pt × ~1.33 ≈ 24px)

    for i, (lb, vl) in enumerate(info_rows, 0):
        r = ROW_INFO_START + i
    ...
    row_data   = row_hdrs + 1              # primeira linha de dados
...
def _criar_resumo(wb, periodo, resultados):
    ...
    for res in resultados:
        ...
        # Positivação da loja = sobre o que a REDE comprou (n_comp_rede), não sobre n_cat
        # Ex: rede comprou 9 itens, loja comprou 5 → 5/9 = 55.6%
...
def gerar_relatorio(fab_nome, redes_selecionadas, data_inicio, data_fim):
    ...
    for rede_nome in redes_selecionadas:
        ...
        for p in prods_cat:
            ...
            if vd and vd["valor_total"] > 0:   # comprado = faturamento real (valor > 0); bonificações R$0 → sem venda
